...
def check_sensitive(input, zz_sensitive_words, sq_sensitive_words, key, llm_kwargs):
    ...
    payload = {
        "model": llm_kwargs['llm_model'].strip('siliconflow-'),
        "messages": messages,
        "temperature": 0.0,  # 1.0,
        "top_p": 0.7,  # 1.0,
        "stream": True
...
def generate_payload(inputs:str, llm_kwargs:dict, history:list, system_prompt:str, image_base64_array:list=[], has_multimodal_capacity:bool=False, stream:bool=True):
    ...
    print("config_model:", model)
    if llm_kwargs['llm_model'].startswith('siliconflow-'):
        model = llm_kwargs['llm_model'][len('siliconflow-'):]

    logging.info("selected model:" + model)
    # 非常关键的操作，top_p不能为0，否则不出结果。
    payload = {
        "model": model,
        "messages": messages,
        "temperature": 0.0,  # 1.0,
